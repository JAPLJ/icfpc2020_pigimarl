...
def mechanical_energy(x, y, vx, vy):
    kinetic = 1/2*(vx)*(vx) + 1/2*(vy)*(vy)
    potential = abs(x) + abs(y)
    return kinetic + potential


def more_energy_moves(x0, y0, vx0, vy0):
    candidate = []
    current_eng = mechanical_energy(x0, y0, vx0, vy0)
    for dx, dy in neighbours:
        vx = vx0 + dx
        vy = vy0 + dy
        next_eng = mechanical_energy(x0, y0, vx, vy)
        if next_eng > current_eng:
            candidate.append([dx, dy])
